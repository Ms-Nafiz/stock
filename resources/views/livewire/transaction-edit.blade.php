<div>
    <div class="p-6 relative">
        <h2 class="text-center font-semibold mb-3 text-xl">Edit Transactions</h2>
        <svg wire:click="$dispatch('closeModal')" class="absolute right-4 top-4 cursor-pointer"
        ...
        <form wire:submit.prevent="editTransaction">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div>
                    ...
                    <input wire:model.defer='nuid' type="text" id="nuid"
                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm cursor-not-allowed" disabled>
                </div>
            ...
            @if (session()->has('message'))
                <div class="text-center mt-2">
                    <span
                        class="p-1  bg-green-100 text-green-600 font-semibold rounded">{{ session('message') }}</span>
                </div>
            @endif
            <!-- Submit Button -->
            <div class="mt-6">
